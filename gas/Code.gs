...
function doGet(e) {
  try {
    ...
    if (action === 'gcal_events') {
      return jsonResponse(getGoogleCalendarEvents(e.parameter));
    }
...
// Called via GET ?action=gcal_events&start=ISO&end=ISO
// ═══════════════════════════════════════════════════════════════════════════

function getGoogleCalendarEvents(params) {
  try {
    const startStr = params.start || new Date().toISOString();
    ...
    const result = events.map(function(evt) {
      ...
      return {
        title:  evt.getTitle(),
        start:  isAllDay
          ? Utilities.formatDate(evt.getAllDayStartDate(), Session.getScriptTimeZone(), 'yyyy-MM-dd')
          : evt.getStartTime().toISOString(),
        end:    isAllDay
          ? Utilities.formatDate(evt.getAllDayEndDate(), Session.getScriptTimeZone(), 'yyyy-MM-dd')
          : evt.getEndTime().toISOString(),
        allDay: isAllDay,
        id:     evt.getId(),
      };
    });
    ...
    return { error: 'Calendar access failed: ' + err.message, events: [] };
  }
}
